...
public class SplitAddChangeVisitor implements ISplitChangeVisitor {
		...
		try {
			for (IJavaElement elem : root.getChildren()) {
				if (elem instanceof IPackageFragment) {
					System.out.println("pkg:" + root.getElementName()
							+ elem.getElementName());
		...
		for (IType type : types) {
			if (!type.getElementName().startsWith("Test")) {
				...
				for (TypeDeclaration tn : typeNodes) {
					if (tn.getName().toString().equals(type.getElementName())) {
						tnode = tn;
						break;
					}
				}
...
//						+ method.getElementName());
	...
	public void visit(IMethod method, MethodDeclaration mnode) {
		...
		if (b == null) {
			AtomicChange c = new AtomicChange(Constants.AM, method);
		...
		GlobalData.atomicData.add(c);
		findLCChanges(method.getDeclaringType(), method, "AM");
	}

	@Override
	public void visit(IField field) {
		AtomicChange c = new AtomicChange(Constants.AF, field);
		GlobalData.atomicData.add(c);

		findLCFChanges(field.getDeclaringType(), field, "AF");
	}
